<template>
  <div class="access-security">
    <a-card :bordered="false">
      <template #title>访问密钥管理</template>
      <template #extra>
      ...
      >
        <template #bodyCell="{ column, record }">
          ...
          <template v-else-if="column.key === 'accessKey'">
            <span>{{ maskSensitiveData(record.accessKey) }}</span>
          </template>
          <template v-else-if="column.key === 'secretKey'">
            <span>{{ maskSensitiveData(record.secretKey) }}</span>
          </template>
...
// 表格数据和分页
const tableData = ref<ConfigAccessKeys[]>([]);
...
const modalTitle = ref('新增访问密钥');
const formRef = ref<FormInstance>();
const form = reactive<Partial<ConfigAccessKeys>>({
  id: undefined,
  componentCode: '',
  accessKey: '',
  secretKey: '',
...
const handleSearch = () => {
  ...
  const searchParams: ConfigAccessKeysParams = {
  ...
  Object.keys(searchParams).forEach((key) => {
    const value = searchParams[key as keyof ConfigAccessKeysParams];
    // 只删除 undefined、null 和空字符串，保留 false
    if (value === '' || value === undefined || value === null) {
      delete searchParams[key as keyof ConfigAccessKeysParams];
    }
  });
...
// 获取数据
const fetchData = async (params?: ConfigAccessKeysParams) => {
  loading.value = true;
  try {
    const defaultParams: ConfigAccessKeysParams = {
      pageNo: pagination.current,
      pageSize: pagination.pageSize,
      sort: '-createdDate',
      ...params,
    ...
    const response = await getConfigAccessKeys(defaultParams);
    tableData.value = response.data || [];
    pagination.total = response.total || 0;
  } catch (error) {
    console.error('获取访问密钥列表失败:', error);
...
// 编辑
const handleEdit = (record: ConfigAccessKeys) => {
  modalTitle.value = '编辑访问密钥';
...
const handleModalOk = async () => {
  try {
    ...
    if (form.id) {
      // 更新
      await updateConfigAccessKeys(form.id, form);
      message.success('更新成功');
    } else {
      // 新增
      await createConfigAccessKeys(form);
...
// 切换启用状态
const handleToggleEnabled = async (record: ConfigAccessKeys, checked: boolean) => {
  toggleLoading.value[record.id] = true;
  try {
    if (checked) {
      await enableConfigAccessKeys(record.id);
    } else {
      await disableConfigAccessKeys(record.id);
    }
...
// 删除
const handleDelete = async (record: ConfigAccessKeys) => {
  try {
    await deleteConfigAccessKeys(record.id);
