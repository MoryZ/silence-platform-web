...
import { getConfigItems, deleteConfigItem, updateConfigContent, createConfigItem } from '../../api/config/configItem';
import type { ConfigItem, ConfigItemParams } from '../../api/config/configItem';
...
const fetchConfigItems = async () => {
  ...
  try {
    ...
    const {total: total, data: data} = await getConfigItems({
...
onMounted(async () => {
  try {
    if (route.path.includes('/cc-config')) {
      // 首先加载子系统列表
