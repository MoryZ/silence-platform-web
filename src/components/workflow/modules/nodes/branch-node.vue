...
interface Emits {
  (e: 'update:modelValue', modelValue: NodeModelType): void;
}
...
const reData = (data: NodeModelType, addData: NodeModelType) => {
  if (!data.childNode) {
...
const arrTransfer = (index: number, type: number = 1) => {
  ...
  nodeConfig.value.conditionNodes?.map((item: any, i: number) => (item.priorityLevel = i + 1));
  emit('update:modelValue', nodeConfig.value);
};

const toText = (node: NodeModelType, currentIndex: number) => {
          ...
          <div class="condition-node">
            <div class="condition-node-box">
              <div class="auto-judge" :class="getClass(item)" @click="showDetail(item, index)">
                ...
                <a-tooltip v-if="item.taskBatchStatus" :title="taskBatchStatusEnum[item.taskBatchStatus].title">
                  <div
